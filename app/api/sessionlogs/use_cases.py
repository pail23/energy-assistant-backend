...
class ReadAllLogEntries:
    ...
    async def execute(
        self, device_id: uuid.UUID | None
    ) -> AsyncIterator[SessionLogEntrySchema]:
        """Execute the read all devices use case."""
        async with self.async_session() as session:
            if device_id is not None:
                async for entry in SessionLogEntry.read_by_device_id(
                    session, device_id
                ):
                    yield SessionLogEntrySchema(
                        ...
                        consumed_energy=entry.end_consumed_energy
