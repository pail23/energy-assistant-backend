        ...
        async with self.async_session() as session:
            if device_id is not None:
                async for entry in SessionLogEntry.read_by_device_id(session, device_id):
                    yield SessionLogEntrySchema(start=entry.start, end=entry.end, text=entry.text, device_id=entry.device_id, solar_consumed_energy=entry.end_solar_consumed_energy-entry.start_solar_consumed_energy, consumed_energy=entry.end_consumed_energy-entry.start_consumed_energy)
            else:
