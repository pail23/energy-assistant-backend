...
def get_injection_dict(df, plot_size=1366):
    ...
    injection_dict[
        "subsubtitle2"
    ] = "<h4>Cost totals for latest optimization results</h4>"
...
def build_params(params, options, addon):
    if addon == 1:
        ...
        params["retrieve_hass_conf"][1]["days_to_retrieve"] = options[
            "historic_days_to_retrieve"
        ]
        params["retrieve_hass_conf"][2]["var_PV"] = options[
            "sensor_power_photovoltaics"
        ]
        params["retrieve_hass_conf"][3]["var_load"] = options[
            "sensor_power_load_no_var_loads"
        ]
        params["retrieve_hass_conf"][6]["var_replace_zero"] = [
            options["sensor_power_photovoltaics"]
        ]
        params["retrieve_hass_conf"][7]["var_interp"] = [
            options["sensor_power_photovoltaics"],
            options["sensor_power_load_no_var_loads"],
        ]
        ...
        params["optim_conf"][3]["P_deferrable_nom"] = [
            i["nominal_power_of_deferrable_loads"]
            for i in options["list_nominal_power_of_deferrable_loads"]
        ]
        params["optim_conf"][4]["def_total_hours"] = [
            i["operating_hours_of_each_deferrable_load"]
            for i in options["list_operating_hours_of_each_deferrable_load"]
        ]
        params["optim_conf"][5]["treat_def_as_semi_cont"] = [
            i["treat_deferrable_load_as_semi_cont"]
            for i in options["list_treat_deferrable_load_as_semi_cont"]
        ]
        params["optim_conf"][6]["set_def_constant"] = [
            False for i in range(len(params["optim_conf"][3]["P_deferrable_nom"]))
        ]
        params["optim_conf"][8]["load_forecast_method"] = options[
        ...
        params["optim_conf"][14]["prod_sell_price"] = options[
        ...
        params["optim_conf"][18]["set_nocharge_from_grid"] = options[
            "set_nocharge_from_grid"
        ]
        params["optim_conf"][19]["set_nodischarge_to_grid"] = options[
        ...
        params["plant_conf"][1]["module_model"] = [
            i["pv_module_model"] for i in options["list_pv_module_model"]
        ]
        params["plant_conf"][2]["inverter_model"] = [
            i["pv_inverter_model"] for i in options["list_pv_inverter_model"]
        ]
        params["plant_conf"][3]["surface_tilt"] = [
            i["surface_tilt"] for i in options["list_surface_tilt"]
        ]
        params["plant_conf"][4]["surface_azimuth"] = [
            i["surface_azimuth"] for i in options["list_surface_azimuth"]
        ]
        params["plant_conf"][5]["modules_per_string"] = [
            i["modules_per_string"] for i in options["list_modules_per_string"]
        ]
        params["plant_conf"][6]["strings_per_inverter"] = [
            i["strings_per_inverter"] for i in options["list_strings_per_inverter"]
        ]
        ...
        params["plant_conf"][14]["SOCtarget"] = options[
...
def index():
    ...
    return make_response(
        template.render(injection_dict=injection_dict, basename=basename)
    )
...
if __name__ == "__main__":
    ...
    app.logger.info(
        "Launching the emhass webserver at: http://" + web_ui_url + ":" + str(port)
    )
    app.logger.info(
        "Home Assistant data fetch will be performed using url: " + hass_url
    )
