...
class retrieve_hass:
    r"""Retrieve data from Home Assistant using the restful API.
    ...
    def get_data(
        ...
        r"""Retrieve the actual data from hass.
        ...
        for day in days_list:
            for i, var in enumerate(var_list):
                ...
                if len(df_raw) == 0:
                    self.logger.error(
                        "Retrieved empty Dataframe, check that correct day or variable names are passed"
                    )
                    self.logger.error(
                        "Either the names of the passed variables are not correct or days_to_retrieve is larger than the recorded history of your sensor (check your recorder settings)"
                    )
    ...
    def prepare_data(
        ...
        r"""Apply some data treatment in preparation for the optimization task.
    ...
    def post_data(
        ...
        if type_var == "power":
            data = retrieve_hass.get_attr_data_dict(
                data_df,
                idx,
                entity_id,
                unit_of_measurement,
