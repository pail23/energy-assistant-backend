...
class optimization:
    r"""Optimize the deferrable load and battery energy dispatch problem using \
    ...
    def __init__(
        ...
        r"""Define constructor for optimization class.
        ...
        self.lp_solver = optim_conf.get("lp_solver", "PULP_CBC_CMD")
    ...
    def perform_optimization(
        ...
        r"""Perform the actual optimization using linear programming (LP).
        ...
        P_grid_neg = {
            (i): plp.LpVariable(
                cat="Continuous",
                lowBound=-self.plant_conf["P_grid_max"],
                upBound=0,
        ...
        P_grid_pos = {
            (i): plp.LpVariable(
                cat="Continuous",
                lowBound=0,
                upBound=self.plant_conf["P_grid_max"],
        ...
        if self.optim_conf["set_use_battery"]:
            P_sto_pos = {
                (i): plp.LpVariable(
                    cat="Continuous",
                    lowBound=0,
                    upBound=self.plant_conf["Pd_max"],
            ...
            P_sto_neg = {
                (i): plp.LpVariable(
                    cat="Continuous",
                    lowBound=-self.plant_conf["Pc_max"],
                    upBound=0,
        ...
        for i in set_I:
            P_def_sum.append(
                plp.lpSum(
                    P_deferrable[k][i] for k in range(self.optim_conf["num_def_loads"])
                )
            )
        ...
        if self.costfun == "self-consumption":
            if type_self_conso == "maxmin":  # maxmin linear problem
                constraints.update(
                    {
                        "constraint_selfcons_PV_{}".format(i): plp.LpConstraint(
        ...
        for k in range(self.optim_conf["num_def_loads"]):
                ...
                constraints.update(
                    {
                        "constraint_pdef{}_semicont1_{}".format(k, i): plp.LpConstraint(
                            e=P_deferrable[k][i]
                            - self.optim_conf["P_deferrable_nom"][k] * P_def_bin1[k][i],
            ...
            if self.optim_conf["set_def_constant"][k]:
                constraints.update(
                    {
                        "constraint_pdef{}_start1".format(k): plp.LpConstraint(
                            e=P_def_start[k][0], sense=plp.LpConstraintEQ, rhs=0
                        )
                    }
                )
                constraints.update(
                ...
                constraints.update(
                    {
                        "constraint_pdef{}_start4_{}".format(k, i): plp.LpConstraint(
                ...
                constraints.update(
                    {
                        "constraint_pdef{}_start5_{}".format(k, i): plp.LpConstraint(
            ...
            constraints.update(
                {
                    "constraint_socmax_{}".format(i): plp.LpConstraint(
                        e=-plp.lpSum(
                            P_sto_pos[j] * (1 / self.plant_conf["eta_disch"])
                            + self.plant_conf["eta_ch"] * P_sto_neg[j]
                            for j in range(i)
                        ),
                        sense=plp.LpConstraintLE,
                        rhs=(self.plant_conf["Enom"] / self.timeStep)
                        * (self.plant_conf["SOCmax"] - soc_init),
                    )
                    for i in set_I
                }
            )
            constraints.update(
                {
                    "constraint_socmin_{}".format(i): plp.LpConstraint(
                        ...
                        sense=plp.LpConstraintLE,
                        rhs=(self.plant_conf["Enom"] / self.timeStep)
                        * (soc_init - self.plant_conf["SOCmin"]),
                    )
                    for i in set_I
                }
            )
            constraints.update(
                {
                    "constraint_socfinal_{}".format(0): plp.LpConstraint(
                        e=plp.lpSum(
                            P_sto_pos[i] * (1 / self.plant_conf["eta_disch"])
                            + self.plant_conf["eta_ch"] * P_sto_neg[i]
                            for i in set_I
                        ),
                        sense=plp.LpConstraintEQ,
                        rhs=(soc_init - soc_final)
                        * self.plant_conf["Enom"]
                        / self.timeStep,
                    )
                }
            )
        ...
        try:
            ...
            self.logger.error(
                "It was not possible to find a valid solver for Pulp package"
            )
        ...
        if plp.value(opt_model.objective) is None:
            ...
            self.logger.info(
                ...
                + str(round(plp.value(opt_model.objective), 2))
            )
        ...
        if self.optim_conf["set_use_battery"]:
            ...
            SOC_opt_delta = [
                (
                    P_sto_pos[i].varValue * (1 / self.plant_conf["eta_disch"])
                    + self.plant_conf["eta_ch"] * P_sto_neg[i].varValue
                )
                * (self.timeStep / (self.plant_conf["Enom"]))
                for i in set_I
            ]
    ...
    def perform_perfect_forecast_optim(
        ...
        r"""Perform an optimization on historical data (perfectly known PV production).
    ...
    def perform_dayahead_forecast_optim(
        ...
        r"""Perform a day-ahead optimization task using real forecast data. \
    ...
    def perform_naive_mpc_optim(
        ...
        r"""Perform a naive approach to a Model Predictive Control (MPC). \
